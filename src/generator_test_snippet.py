...
def lindblad_mpo_latex(config_kwargs, sym='dense', N=4, gamma=np.ones([4,4])):
    ...
    ltx_str_simple = r"-imun (\sum_{j \in Nx} (z_{j} - zcc_{j})) + \sum_{j,k \in NxN} gamma_{j,k} (z_{j} zcc_{k} - 1.0div2.0 ( z_{k} z_{j} + zcc_{k} zcc_{j} ) )"
    parameters = {"imun": 1j,
                  "gamma": gamma,
                  "1.0div2.0": 1/2,
                  "N": N,
                  "Nx": list(range(N)),
                  "NxN": [(i,j) for i in range(N) for j in range(N)]}
